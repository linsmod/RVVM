...
/* Key codes (android/keycodes.h). VirtPass carries the subset the hosts can
 * actually produce: the Win32 host maps its VK_* onto these, the Android host
 * receives them from the framework unchanged. The values are Android's own, so
 * a guest comparing against AKEYCODE_* behaves identically on either host. */
...
/* Key event actions (android/keycodes.h) and the meta flags carried with
 * them in GameActivityKeyEvent::metaState. */
#define AKEY_EVENT_ACTION_DOWN           0
#define AKEY_EVENT_ACTION_UP             1
#define AKEY_EVENT_ACTION_MULTIPLE       2

#define AMETA_SHIFT_ON                   0x00000001
#define AMETA_ALT_ON                     0x00000002
#define AMETA_CTRL_ON                    0x00001000
#define AMETA_META_ON                    0x00010000
